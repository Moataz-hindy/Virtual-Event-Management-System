    ...
    while (true) {
        ...
        switch (choice) {
            case 1:
                ...
                event = Event::eventBuilder(Conference_, logged_user);
                break;
            case 2:
                // Usage of Factory Design Pattern
                event = Event::eventBuilder(Webinar_, logged_user);
                break;
            case 3:
                // Usage of Factory Design Pattern
                event = Event::eventBuilder(Workshop_, logged_user);
                break;
            case 4:
...
///////////////////////////   Event   /////////////////////////
Event::Event(){}
Event::Event(string n, string desc, string p, string d, string t, string creator, int c, int f)
    : event_name(n), description(desc), platform(p), date(d), time(t), creator(creator), capacity(c), full(f) {}
    ...
    while (true) {
        ...
        if (found) {
            ...
            for (Event* e: allEvents) {
                name = e->getName();
                if (name == eventName) {
                    e->notify(Event_cancellation);
                    e->detach(e->getCreator());
                }
            }
            return;
        } else {
...
Event* Event::eventBuilder(const eventType Type, const string& logged_user) {
    if (Type == Conference_) {
        return Conference::create_event(logged_user);
    }
    else if (Type == Webinar_) {
        return Webinar::create_event(logged_user);
    }
    else if (Type == Workshop_) {
        return Workshop::create_event(logged_user);
    }
    else return nullptr;
}
...
Conference::Conference(string n, string desc, string p, string d, string t, string creator, int c, int dur)
    : Event(n, desc, p, d, t, creator, c, 0), duration(dur) {}
...
Event* Conference::create_event(string logged_user){
    ...
    Conference* conference = new Conference(name, desc, platform, date, time, logged_user, capacity, duration);
    return conference;
}
...
Event* Conference::loadFromFile(const string& line){
    ...
    Conference* conference = new Conference(name, desc, platform, date, time, fileUser, capacity, duration);

    // Attach all registrars as observers (do NOT attach creator)
    while(getline(ss, registrar, '|')){
        conference->addRegistrar(registrar);
        for (const User& user : Users) {
            if (user.getUsername() == registrar) {
                conference->attach(const_cast<User*>(&user));
                break;
            }
        }
    }
...
/////////////////////////   Webinar   ///////////////////////////
Webinar::Webinar(){}
Webinar::Webinar(string n, string desc, string p, string d, string t, string creator, int c, string h)
    : Event(n, desc, p, d, t, creator,  c, 0), host(h) {}
...
// Static Factory Method Implementation: Part of Factory Design Pattern
Event* Webinar::create_event(string logged_user){
    ...
    Webinar* webinar = new Webinar(name, desc, platform, date, time, logged_user, capacity, host);
    return webinar;
}
...
Event* Webinar::loadFromFile(const string& line){
    ...
    getline(ss, host, '|');
    int capacity = stoi(capacityStr);
    Webinar* webinar = new Webinar(name, desc, platform, date, time, fileUser, capacity, host);
    ...
    while(getline(ss, registrar, '|')){
        ...
        for (const User& user : Users) {
            if (user.getUsername() == registrar) {
                webinar->attach(const_cast<User*>(&user));
                break;
            }
        }
    }
...
////////////////////////////   Workshop   ////////////////////////
Workshop::Workshop(){}
Workshop::Workshop(string n, string desc, string p, string d, string t, string creator, int c, string i)
    : Event(n, desc, p, d, t, creator, c, 0), instructor(i) {}
...
// Static Factory Method Implementation: Part of Factory Design Pattern
Event* Workshop::create_event(string logged_user){
    ...
    Workshop* workshop = new Workshop(name, desc, platform, date, time, logged_user, capacity, instructor);
    return workshop;
}
...
Event* Workshop::loadFromFile(const string& line){
    ...
    int capacity = stoi(capacityStr);
    Workshop* workshop = new Workshop(name, desc, platform, date, time, fileUser, capacity, instructor);
    ...
    while(getline(ss, registrar, '|')){
        ...
        for (const User& user : Users) {
            if (user.getUsername() == registrar) {
                workshop->attach(const_cast<User*>(&user));
                break;
            }
        }
    }
...
void Event::register_event(const string& logged_user){
    ...
    if (!selectedEvent->addRegistrar(logged_user)) {
        ...
        // Find the matching event in allEvents and attach the observer there
        for (Event* e : allEvents) {
            if (
                e->getName() == selectedEvent->getName() &&
                e->getDate() == selectedEvent->getDate() &&
                e->getTime() == selectedEvent->getTime()
            ) {
                for (const User& user : Users) {
                    if (user.getUsername() == logged_user) {
                        e->attach(const_cast<User*>(&user));
                        break;
                    }
                }
                break;
...
void Feedback::attach(Observer* observer) { observer_ = observer; }

void Feedback::detach(Observer* observer) { observer_ = nullptr; }

void Feedback::notify() {
        observer_->update(eventName_, "New feedback received by " + reviewerUsername_);
}
